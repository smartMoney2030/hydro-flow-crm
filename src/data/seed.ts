...
export const users: User[] = [
  ...
  },
  {
    id: "u6",
    name: "Jordan Kim",
    email: "jordan@mywaterpeople.com",
    role: "technician",
    avatarColor: "#d97706",
    active: true,
    phone: "(210) 555-0106",
...
export const leads: Lead[] = customers.slice(0, 28).map((c, i) => {
  ...
  return {
    ...
    currentEquipment:
      i % 3 === 0 ? "None" : i % 3 === 1 ? "Old softener (10+ yrs)" : "Under-sink filter",
    salesCallAt: status !== "New Lead" ? iso(-(i % 14) + 2, 10 + (i % 6)) : undefined,
    followUpAt: status === "Follow-Up" ? iso(2 + (i % 5)) : undefined,
    quoteAmount: ["Quote Sent", "Follow-Up", "Sale Won", "Sale Lost"].includes(status)
      ? quoteAmount
      : undefined,
    quoteStatus:
      status === "Sale Won"
        ...
        : status === "Sale Lost"
          ? "Declined"
          : status === "Quote Sent" || status === "Follow-Up"
            ? "Sent"
            : "Draft",
    lostReason:
      status === "Sale Lost" ? ["Price", "Went with competitor", "Not ready"][i % 3] : undefined,
    notes: "",
    createdAt: iso(-(i * 2 + 3)),
  };
});
...
export const jobs: Job[] = Array.from({ length: 22 }, (_, i) => {
  ...
  return {
    ...
    paymentStatus:
      status === "Payment Pending" ? "Unpaid" : status === "Job Closed" ? "Paid" : "Deposit Paid",
...
export const supplyOrders: SupplyOrder[] = jobs
  ...
  .map((j, i) => ({
    ...
    status:
      j.status === "Supplies Ordered"
        ? "Ordered"
        : j.status === "Awaiting Delivery"
          ? "In Transit"
          : "Delivered",
    notes: "",
  }));
...
export const installations: Installation[] = jobs
  ...
  .map((j, i) => {
    ...
    return {
      ...
      status:
        j.status === "Installation Scheduled"
          ? "Scheduled"
          : j.status === "Installation in Progress"
            ? "In Progress"
            : "Completed",
      ...
      serials: done
        ? [`SN-${(700000 + i * 13).toString()}`, `SN-${(800000 + i * 17).toString()}`]
        : [],
  ...
  .flatMap((inst, idx) => [
    {
      ...
      warrantyExpires: new Date(
        new Date(inst.completedAt!).getTime() + 365 * 5 * DAY,
      ).toISOString(),
    ...
    },
    {
      ...
      installDate: inst.completedAt!,
      warrantyExpires: new Date(
        new Date(inst.completedAt!).getTime() + 365 * 3 * DAY,
      ).toISOString(),
      status: "Active" as const,
      lastMaintenance: undefined,
      nextMaintenance: new Date(new Date(inst.completedAt!).getTime() + 365 * DAY).toISOString(),
...
export const maintenanceVisits: MaintenanceVisit[] = equipment.slice(0, 24).map((eq, i) => {
  ...
  return {
    ...
    scheduledAt:
      status === "Maintenance Scheduled" || status === "Maintenance Completed"
        ? iso(dueOffset, 10)
        : undefined,
    technicianId:
      status === "Maintenance Scheduled" || status === "Maintenance Completed"
    ...
    workPerformed:
      status === "Maintenance Completed"
        ? "Replaced sediment filter, sanitized, tested hardness (0 gpg)."
        : undefined,
...
export const tasks: Task[] = [
  {
    ...
    description: "Home in Alamo Heights, hard water complaint",
    dueAt: iso(0, 14),
    assigneeId: "u2",
    relatedLeadId: "l1",
    priority: "high",
  ...
  {
    id: "t3",
    title: "Collect deposit — Job INV-2024005",
    dueAt: iso(0, 16),
    assigneeId: "u2",
    priority: "high",
    done: false,
  ...
  {
    id: "t4",
    title: "Order supplies — INV-2024007",
    dueAt: iso(-1, 9),
    assigneeId: "u4",
    priority: "high",
    done: false,
  ...
  {
    id: "t6",
    title: "Follow-up call after install",
    dueAt: iso(3, 15),
    assigneeId: "u2",
    priority: "low",
    done: false,
  ...
  {
    id: "t7",
    title: "Confirm maintenance appt — Lee",
    dueAt: iso(1, 9),
    assigneeId: "u4",
    priority: "med",
    done: false,
  ...
  {
    id: "t8",
    title: "Restock van — softener resin",
    dueAt: iso(0, 8),
    assigneeId: "u5",
    priority: "med",
    done: true,
...
export const events: CalendarEvent[] = [
  ...
  ...maintenanceVisits
    .filter((m) => m.status === "Maintenance Scheduled")
    .map((m) => ({
      id: `ev-m-${m.id}`,
  ...
  ...leads
    .filter((l) => l.salesCallAt && daysAhead(l.salesCallAt) > -3 && daysAhead(l.salesCallAt) < 14)
    .map((l) => ({
      id: `ev-sc-${l.id}`,
...
export const notifications: Notification[] = [
  {
    ...
    body: "James Garcia submitted the contact form.",
    createdAt: iso(0, 8),
    read: false,
    kind: "info",
    href: "/leads",
  },
  {
    ...
    body: "INV-2024003 · $780 collected.",
    createdAt: iso(0, 10),
    read: false,
    kind: "success",
    href: "/jobs-pipeline",
  },
  {
    ...
    body: "3 customers are past due — flag on dashboard.",
    createdAt: iso(-1, 9),
    read: false,
    kind: "warning",
    href: "/maintenance",
  },
  {
    ...
    body: "AquaWorks tracking 1Z999… marked delivered.",
    createdAt: iso(-1, 14),
    read: true,
    kind: "success",
    href: "/supply-orders",
...
export const auditLogs: AuditLog[] = [
  ...
  },
  {
    ...
    entityId: "u3",
    at: iso(-3, 10, 20),
    detail: "Salesperson permissions updated",
  },
];
...
export const automationRules: AutomationRule[] = [
  {
    ...
    effect: "Create customer + lead, assign salesperson, create sales-call task, notify",
    enabled: true,
    lastRunAt: iso(0, 8),
    runsToday: 3,
  },
  {
    id: "ar2",
    name: "Sale won → Job",
    trigger: "Lead moves to Sale Won",
    effect: "Create job, create payment task, preserve lead history",
    enabled: true,
    lastRunAt: iso(-1, 15),
    runsToday: 1,
  },
  {
    ...
    trigger: "Required payment received",
    effect: "Move job to Supplies Need Ordering, create equipment-order task",
    enabled: true,
    lastRunAt: iso(0, 10),
    runsToday: 2,
  },
  {
    ...
    trigger: "All supplies marked delivered",
    effect: "Move job to Ready to Schedule, notify scheduler",
    enabled: true,
    lastRunAt: iso(-1, 14),
    runsToday: 1,
  },
  {
    ...
    trigger: "Installation appointment saved",
    effect: "Create calendar event, customer confirmation, reminders; Google Cal sync",
    enabled: true,
    lastRunAt: iso(-1, 11),
    runsToday: 4,
  },
  {
    ...
    trigger: "Technician completes install",
    effect: "Create equipment records, save serials/photos/warranty, first maintenance +1yr",
    enabled: true,
    lastRunAt: iso(-2, 16),
    runsToday: 0,
  },
  {
    ...
    trigger: "60/30/7 days before due",
    effect: "Notify customer + create scheduler task; overdue records flagged",
    enabled: true,
    lastRunAt: iso(0, 6),
    runsToday: 6,
  },
  {
    ...
    trigger: "Maintenance visit completed",
    effect: "Save service history, set next maintenance +1yr",
    enabled: true,
    lastRunAt: iso(-2, 12),
    runsToday: 0,
  },
...
export const automationRuns: AutomationRun[] = [
  {
    ...
    at: iso(0, 8, 12),
    status: "success",
    detail: "Created c1 / l1, task t1, notified Maya",
  },
  {
    id: "arn2",
    ruleId: "ar3",
    at: iso(0, 10, 3),
    status: "success",
    detail: "Job INV-2024003 → Supplies Need Ordering",
  },
  {
    id: "arn3",
    ruleId: "ar4",
    at: iso(-1, 14, 22),
    status: "success",
    detail: "Job INV-2024001 → Ready to Schedule",
  },
  {
    id: "arn4",
    ruleId: "ar7",
    at: iso(0, 6, 0),
    status: "success",
    detail: "6 reminders queued (3× 30-day, 2× 7-day, 1× overdue)",
  },
  {
    id: "arn5",
    ruleId: "ar5",
    at: iso(-1, 11, 10),
    status: "success",
    detail: "Install for Perez synced to calendar",
  },
  {
    id: "arn6",
    ruleId: "ar2",
    at: iso(-1, 15, 40),
    status: "success",
    detail: "Lead l7 converted → Job INV-2024015",
  },
];
