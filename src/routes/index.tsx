...
function Dashboard() {
  ...
  const salesCallsDue = s.leads.filter(
    (l) =>
      l.salesCallAt &&
      new Date(l.salesCallAt).getTime() < now + 2 * DAY &&
      new Date(l.salesCallAt).getTime() > now - DAY,
  ).length;
  const quotesOut = s.leads.filter((l) => l.status === "Quote Sent").length;
  const wonThisMonth = s.jobs.filter(
    (j) => new Date(j.saleDate).getMonth() === new Date().getMonth(),
  ).length;
  ...
  const ordersInTransit = s.supplyOrders.filter(
    (o) => o.status === "In Transit" || o.status === "Ordered",
  ).length;
  const readyToSchedule = s.jobs.filter((j) => j.status === "Ready to Schedule").length;
  const installsThisWeek = s.installations.filter(
    (i) => new Date(i.startAt).getTime() > now && new Date(i.startAt).getTime() < now + 7 * DAY,
  ).length;
  const maint30 = s.maintenance.filter((m) =>
    ["Due Within 30 Days", "Due Within 7 Days"].includes(m.status),
  ).length;
  const overdue = s.maintenance.filter((m) => m.status === "Maintenance Overdue").length;
  const revenue = s.jobs
    .filter((j) => new Date(j.saleDate).getMonth() === new Date().getMonth())
    .reduce((a, j) => a + j.totalPrice, 0);
  const totalLeads = s.leads.length;
  const conv = Math.round(
    (s.leads.filter((l) => l.status === "Sale Won").length / Math.max(totalLeads, 1)) * 100,
  );
  ...
  const techWorkload = s.users
    .filter((u) => u.role === "technician")
    .map((u) => ({
      name: u.name.split(" ")[0],
      installs: s.installations.filter((i) => i.technicianId === u.id && i.status !== "Completed")
        .length,
      maintenance: s.maintenance.filter(
        (m) => m.technicianId === u.id && m.status === "Maintenance Scheduled",
      ).length,
    }));
  ...
  return (
    <>
      <PageHeader
        eyebrow="Overview"
        title="Dashboard"
        description="Everything moving in your business today."
      />
      <Section className="space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
          ...
          <StatCard
            label="Open tasks"
            value={s.tasks.filter((t) => !t.done).length}
            to="/tasks"
            icon="CheckSquare"
          />
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <Card className="shadow-card">
            <CardHeader className="pb-2 flex-row items-center justify-between">
              ...
              <Link to="/installations" className="text-xs text-primary hover:underline">
                View all
              </Link>
            </CardHeader>
              ...
              {upcomingInstalls.map((i) => {
                ...
                return (
                  <Link
                    ...
                    params={{ id: c.id }}
                    key={i.id}
                    className="flex items-center gap-3 p-2 rounded-md hover:bg-muted/60"
                  >
                    <div
                      className="h-9 w-9 rounded-full grid place-items-center text-xs font-semibold text-white shrink-0"
                      style={{ backgroundColor: t?.avatarColor || "#0891b2" }}
                    >
                      {initials(t?.name || "T")}
                    </div>
          ...
          <Card className="shadow-card">
            ...
            <CardContent className="space-y-2">
              {s.maintenance
                .filter((m) =>
                  ["Maintenance Overdue", "Due Within 7 Days", "Due Within 30 Days"].includes(
                    m.status,
                  ),
                )
                .slice(0, 5)
          ...
          <CardContent className="p-5 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4">
            <div className="min-w-0">
              ...
              <div className="mt-2 text-lg font-semibold">
                {s.automationRules.filter((r) => r.enabled).length} of {s.automationRules.length}{" "}
                rules active
              </div>
              <div className="text-sm text-white/80">
                Automations processed {s.automationRules.reduce((a, r) => a + r.runsToday, 0)}{" "}
                events today.
              </div>
            </div>
            <Link
              to="/automations"
              className="rounded-md bg-white text-primary px-4 py-2 text-sm font-medium hover:bg-white/90 shrink-0"
            >
              Open
            </Link>
          </CardContent>
