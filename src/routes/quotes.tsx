    ...
    tiers: [
      {
        name: "Standard",
        price: 3200,
        features: ["Whole-home softener", "Basic install", "1yr warranty"],
      },
      {
        name: "Premium",
        price: 4800,
        features: [
          "Softener + RO drinking",
          "Premium install",
          "5yr warranty",
    ...
    tiers: [
      {
        name: "RO Under-sink",
        price: 1450,
        features: ["4-stage RO", "Faucet included", "Install"],
      },
    ],
    status: "Approved",
    selectedTier: "RO Under-sink",
    depositPct: 50,
    signed: true,
    createdAt: new Date().toISOString(),
  },
  ...
  return (
    <>
      <PageHeader
        ...
        description="Professional quotes with e-signature and deposits"
        actions={<Button size="sm">New quote</Button>}
      />
              ...
              <CardContent className="p-4">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-muted-foreground">
                      Deposit ${deposit.toLocaleString()} ({q.depositPct}%)
                    </div>
                  </div>
                </div>
                <div className="grid sm:grid-cols-2 gap-3 mt-3">
                  {q.tiers.map((t) => (
                    <div
                      key={t.name}
                      className={`border rounded-lg p-3 ${q.selectedTier === t.name ? "border-primary bg-primary/5" : ""}`}
                    >
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => toast.success(`Sent ${q.number} to ${q.email}`)}
                  >
                  ...
                  <Button
                    ...
                    onClick={() => {
                      setQuotes((x) =>
                        x.map((y) =>
                          y.id === q.id ? { ...y, signed: true, status: "Approved" } : y,
                        ),
                      );
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => toast.success(`Deposit of $${deposit} requested`)}
                  >
