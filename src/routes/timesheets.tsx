  ...
  return (
    <>
      <PageHeader
        eyebrow="Payroll"
        title="Timesheets"
        description={`Clock in/out, approve, and confirm payroll · ${currentName} (${currentUser?.role ?? "—"})`}
        actions={
          running ? (
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          <Card>
            <CardContent className="p-4">
              ...
              <div className="text-2xl font-bold">{totalHrs.toFixed(1)}</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="text-xs text-muted-foreground">Est. payroll</div>
              <div className="text-2xl font-bold">${(totalHrs * 32).toFixed(0)}</div>
            </CardContent>
          </Card>
        </div>
        <Card>
            ...
            <div className="p-4 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  variant="outline"
                  onClick={() => {
                    setRows((x) => x.map((y) => ({ ...y, approved: true })));
                    toast.success("All timesheets approved");
                  }}
                >
                  Approve all
                </Button>
                <Button size="sm" onClick={() => toast.success("Payroll confirmed and exported")}>
                  Confirm payroll
                </Button>
              </div>
            </div>
