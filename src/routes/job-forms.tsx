...
const forms: Form[] = [
  {
    id: "f1",
    name: "Softener install checklist",
    type: "Checklist",
    fields: [
  ...
  {
    id: "f2",
    name: "RO drinking install",
    type: "Checklist",
    fields: [
      "Under-sink space verified",
      "Faucet mounted",
      "Membrane flushed",
      "Leak check 15min",
      "TDS reading recorded",
    ],
  },
  {
    id: "f3",
    name: "Annual maintenance form",
    type: "Form",
    fields: [
      "Salt level %",
      "Bypass tested",
      "Resin cleaner applied",
      "Pre/post TDS",
      "Customer signature",
    ],
  },
  {
    ...
    type: "Form",
    fields: ["Hardness (gpg)", "Iron (ppm)", "pH", "Chlorine (ppm)", "TDS", "Recommendation"],
  },
];
  ...
  return (
    <>
      <PageHeader
        ...
        description="Standardize field work with required steps and captured data"
        actions={<Button size="sm">New form</Button>}
      />
          ...
          <CardContent className="p-0 divide-y">
            {forms.map((f) => (
              <button
                ...
                className={`w-full text-left p-3 hover:bg-accent transition ${active.id === f.id ? "bg-accent" : ""}`}
              >
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  {active.type} · attach to any job
                </div>
              </div>
              <Button
                size="sm"
                variant="outline"
                onClick={() => toast.success(`Submitted (${checked.size}/${active.fields.length})`)}
              >
                Submit
              </Button>
            </div>
            <div className="space-y-2">
              {active.fields.map((f) => (
                <label key={f} className="flex items-center gap-3 p-2 border rounded-lg text-sm">
                  ...
                  />
                  <span className={checked.has(f) ? "line-through text-muted-foreground" : ""}>
                    {f}
                  </span>
                </label>
              ))}
