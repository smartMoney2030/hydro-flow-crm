...
function TechHome() {
  ...
  const jobs = useMemo(
    () =>
      s.installations.filter(
        (i) => i.technicianId === uid && new Date(i.startAt).toDateString() === todayKey,
      ),
    [s.installations, todayKey, uid],
  );
  const upcoming = s.installations
    .filter(
      (i) => i.technicianId === uid && new Date(i.startAt) > today && i.status !== "Completed",
    )
    .slice(0, 3);
  ...
  return (
    <div className="min-h-screen bg-gradient-soft pb-8">
      ...
      <div className="p-4 space-y-3">
            ...
            <Card key={i.id} className="shadow-card">
              <CardContent className="p-4 space-y-3">
                ...
                <Button asChild className="w-full bg-primary" size="lg">
                  <Link to="/technician/job/$id" params={{ id: i.id }}>
                    Start Job <ArrowRight className="h-4 w-4 ml-1" />
                  </Link>
                </Button>
              </CardContent>
            </Card>
        ...
        {upcoming.length > 0 && (
          <>
            <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground pt-4">
              Coming up
            </h2>
            {upcoming.map((i) => {
              const c = s.customers.find((c) => c.id === i.customerId)!;
                  ...
                  <CardContent className="p-3 flex items-center justify-between">
                    <div className="min-w-0">
                      <div className="text-xs text-muted-foreground">
                        {new Date(i.startAt).toLocaleDateString()} · {time(i.startAt)}
                      </div>
