  ...
  return (
    <>
      <PageHeader
        ...
        actions={<Button className="bg-primary">+ New lead</Button>}
      />
      <Section className="space-y-4">
        ...
        <div className="grid gap-2">
              ...
              <Card key={l.id} className="shadow-sm hover:shadow-card transition-shadow">
                <CardContent className="p-4">
                  <Link
                    to="/customers/$id"
                    params={{ id: c.id }}
                    className="grid grid-cols-1 md:grid-cols-[minmax(0,1fr)_auto_auto_auto] items-center gap-3"
                  >
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex gap-1 flex-wrap">
                        {l.waterConcerns.map((w) => (
                          <span key={w} className="text-[10px] px-1.5 py-0.5 rounded bg-muted">
                            {w}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="text-sm font-semibold text-primary">
                      {l.quoteAmount ? money(l.quoteAmount) : "—"}
                    </div>
                    <div className="flex flex-col items-end gap-1">
                      <StatusBadge status={l.status} />
                      {l.followUpAt && (
                        <span className="text-[10px] text-muted-foreground">
                          Follow-up {relDays(l.followUpAt)}
                        </span>
                      )}
                    </div>
                  </Link>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-16 text-muted-foreground">No leads match.</div>
          )}
        </div>
      </Section>
    </>
