...
function InvoicesPage() {
  ...
  const totals = rows.reduce(
    (a, r) => ({ owed: a.owed + (r.amount - r.paid), paid: a.paid + r.paid }),
    { owed: 0, paid: 0 },
  );
  return (
    <>
      <PageHeader
        eyebrow="Financials"
        title="Invoices"
        description="One-click convert jobs to invoices"
      ...
      <Section className="space-y-4">
        <div className="grid sm:grid-cols-3 gap-3">
          ...
          <StatCard
            label="Overdue"
            value={rows.filter((r) => r.status === "Overdue").length.toString()}
            tone="danger"
          />
        </div>
        <Card>
                    ...
                    <TableCell className="text-right">
                      <Button
                        ...
                        onClick={() => {
                          setRows((x) =>
                            x.map((y) =>
                              y.id === r.id ? { ...y, paid: y.amount, status: "Paid" } : y,
                            ),
                          );
