          ...
          <div className="flex rounded-md border p-0.5">
            {(["week", "day", "tech"] as const).map((v) => (
              <button
                key={v}
                onClick={() => setView(v)}
                className={`px-3 py-1 text-xs rounded ${view === v ? "bg-primary text-primary-foreground" : ""}`}
              >
                {v}
              </button>
            ))}
          ...
          <div className="grid grid-cols-7 gap-2">
            {days.map((d) => {
              ...
              return (
                <Card
                  key={d.toISOString()}
                  className={`min-h-[280px] ${isSameDay(d, new Date()) ? "border-primary" : ""}`}
                >
                  <CardContent className="p-2">
                    ...
                    <div className="space-y-1">
                      {dayEvents.map((e) => (
                        <div
                          key={e.id}
                          className={`text-[11px] rounded p-1.5 border ${typeColor[e.type]}`}
                        >
                          <div className="font-medium truncate">
                            {format(new Date(e.startAt), "h:mma")}
                          </div>
        ...
        {view === "day" && (
          <Card>
            <CardContent className="p-4 space-y-2">
              {filtered
                .filter((e) => isSameDay(new Date(e.startAt), anchor))
                .map((e) => (
                    ...
                    <div className="grid grid-cols-7 gap-2">
                      {days.map((d) => {
                        const evs = s.events.filter(
                          (e) => e.technicianId === u.id && isSameDay(new Date(e.startAt), d),
                        );
                        return (
                          <div
                            key={d.toISOString()}
                            className="min-h-[80px] rounded bg-muted/40 p-1.5 space-y-1"
                          >
                            ...
                            {evs.map((e) => (
                              <div
                                key={e.id}
                                className={`text-[10px] p-1 rounded border ${typeColor[e.type]} truncate`}
                              >
                                {e.title}
                              </div>
                            ))}
        ...
        <Card>
          <CardContent className="p-4 flex flex-wrap gap-2 items-center text-xs">
            ...
            {Object.entries(typeColor).map(([k, v]) => (
              <Badge key={k} variant="outline" className={v}>
                {k}
              </Badge>
            ))}
            <span className="ml-auto text-muted-foreground">
              Travel buffer: 15 min · Google Calendar sync: <b>Off</b>
            </span>
          </CardContent>
        </Card>
      </Section>
