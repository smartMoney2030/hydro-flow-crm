...
export function csvTemplate(): string {
  ...
  const example = [
    ...
    "(210) 555-9911",
    "jane@example.com",
    "1201 Broadway, San Antonio, TX 78215",
    "1201 Broadway, San Antonio, TX 78215",
    "2022-04-10",
    "2022-04-18",
    ...
    "Tomás Reed",
    "Gate code #1201",
    "Filter swap 2023-10; leak check 2024-04",
    "yes",
    "Active Maintenance Customer",
  ];
...
export function findDuplicates(
  ...
  for (const cust of customers) {
    ...
    let score = 0;
    if (cPhone && digits(cust.phone) === cPhone) {
      reasons.push("Same phone");
      score += 3;
    }
    if (cEmail && norm(cust.email) === cEmail) {
      reasons.push("Same email");
      score += 3;
    }
    if (cAddr && norm(cust.propertyAddress) === cAddr) {
      reasons.push("Same property address");
      score += 3;
    }
    if (cName && norm(`${cust.firstName} ${cust.lastName}`) === cName) {
